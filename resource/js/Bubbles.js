...
function Bubbles(container, self, options) {
  ...
  animationTime = options.animationTime || 1 // how long it takes to animate chat bubble, also set in CSS
    ...
    inputText.addEventListener("keypress", function(e) {
      ...
      if (e.keyCode == 13) {
        ...
        !lastBubble.classList.contains("reply-freeform")
          ...
          : false
        addBubble(
          '<span class="bubble-button bubble-pick bg-dark flex-fill p-2 rounded">' + this.value + "</span>",
          function() {},
  ...
  // init typing bubble
  var bubbleTyping = document.createElement("span")
    ...
    if (turn.reply !== undefined) {
      ...
      for (var i = 0; i < turn.reply.length; i++) {
        ...
        ;(function(el, count) {
          ...
          questionsHTML +=
            '<span class="bubble-button bg-dark flex-fill p-2 rounded border-bottom" style="animation-delay: ' +
            10 +
            'ms" onClick="'+ self + ".answer('" + el.answer +  "', '" + el.question + "');this.classList.add('bubble-pick');"+funcParam+"  \">" + 
            el.question + 
            "</span> <br>"
        })(turn.reply[i], i)
      }
  ...
  var addBubble = function(say, posted, reply, live) {
    ...
    if(reply !== ""){// If user must choose an response
      content.setAttribute('class', 'd-flex justify-content-end w-100')
      
      bubble.setAttribute("class", " d-flex justify-content-around w-100 float-right p-2 rounded");

      var bubbleContent = document.createElement("span")
      bubbleContent.className = "bubble-content d-flex flex-wrap justify-content-around text-center text-white w-100"
    }else{
      content.setAttribute('class', 'd-flex justify-content-start w-75 p-1 mt-0')

      bubble.setAttribute("class", "  p-2 rounded");
      bubble.setAttribute("style", "width: 100%; background: #c21b17 !important;");
      var bubbleContent = document.createElement("span")
      bubbleContent.className = "bubble-content text-center text-white"
        ...
        if (e.target.classList.contains('bubble-button')) {
          for (var i = 0; i < bubbleButtons.length; i++) {
            ;(function(el) {
              ...
              el.classList.contains("bubble-pick") ? (el.style.width = "fit-content") : false
